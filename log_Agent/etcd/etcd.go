...
type LogEntry struct {
	Path  string `json:"path"`  //日志文件的路径
	Topic string `json:"topic"` //指定日志文件放入的topic
}
...
func GetConfig(key string) (logEntryConfs []*LogEntry, err error) {
	...
	for _, v := range resp.Kvs {
		err = json.Unmarshal(v.Value, &logEntryConfs)
		if err != nil {
...
func WatchConf(key string, newConf chan<- []*LogEntry) {
	//watch操作
	...
	for wresp := range resChan {
		...
		for _, v := range wresp.Events {
			...
			if v.Type != clientV3.EventTypeDelete {
				err := json.Unmarshal(v.Kv.Value, &newLogEntry)
				if err != nil {
					log.Fatalln("watchConf unmarshal failed , err : ", err)
					//continue
				}
				fmt.Println("get new conf :", newLogEntry)
				newConf <- newLogEntry
			}
